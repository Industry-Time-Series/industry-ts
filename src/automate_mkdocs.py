...
def automate_mkdocs_from_docstring(
    ...
    """
    p = repo_dir.glob('**/*.py')
    scripts = [x for x in p if x.is_file() and "automate_mkdocs" not in str(x)]
    ...
    with open(f'{repo_dir}/{mkgendocs_f}', 'r+', encoding='utf-8') as mkgen_config:
        insert_string = ''
        ...
        if match_string in contents[-1]:
            ...
            for index, line in enumerate(contents):
                if match_string in line and insert_string not in contents[index + 1]:
                    ...
                    contents.append(insert_string)
                    break
    contents = [content.replace("blob/master", "blob/main") for content in contents]
    with open(f'{repo_dir}/{mkgendocs_f}', 'w', encoding='utf-8') as mkgen_config:
...
def automate_nav_structure(
    ...
    with open(f'{repo_dir}/{mkdocs_f}', 'r+', encoding='utf-8') as mkgen_config:
        contents = mkgen_config.readlines()
        if match_string in contents[-1]:
            contents.append(insert_string)
        else:
    ...
    contents = [content.replace("blob/master", "blob/main") for content in contents]
    with open(f'{repo_dir}/{mkdocs_f}', 'w', encoding='utf-8') as mkgen_config:
        mkgen_config.writelines(contents)
...
def main():
    ...
    python_tips_dir = Path.cwd().joinpath('').joinpath("")
